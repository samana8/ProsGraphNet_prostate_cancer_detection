...
def transformAndSaveRegion(preprocess_moving_dest, case, slice, s, region, theta, dH, dW, h, w,x,y,x_offset,y_offset): 
    ...
    case_output_path = os.path.join(preprocess_moving_dest, case)
    # print(f"Case output path: {case_output_path}")
    try: 
        os.mkdir(case_output_path)
    except: 
        pass 
    
    outputPath = os.path.join(case_output_path,  region + '_' + case + '_' + slice +'.png')
...
def preprocess_hist(moving_dict, pre_process_moving_dest, case): 
    # print(f"Moving dict: {moving_dict}, {pre_process_moving_dest}")
...
def preprocess_mri(fixed_img_mha, fixed_seg, pre_process_fixed_dest, coord, case):     
    ...
    for slice in range(imMri.shape[0]):
        ...
        case_output_path = os.path.join(pre_process_fixed_dest, case)
        try: 
            os.mkdir(case_output_path)
        except: 
            pass 
        ...
        # write to a file        
        cv2.imwrite(os.path.join(case_output_path, 'mri_' + case + '_' + str(slice).zfill(2) +'.jpg'), upsMri)

        
        cv2.imwrite(os.path.join(case_output_path, 'mriUncropped_' + case + '_' + str(slice).zfill(2) +'.jpg'), imMri[slice, :, :])
        cv2.imwrite(os.path.join(case_output_path, 'mriMask_' + case + '_' + str(slice).zfill(2) +'.jpg'), np.uint8(mri_mask))
