...
class GeometricTnf(object):
    ...
    def __init__(self, geometric_model='affine', out_h=240, out_w=240, use_cuda=True):
        ...
        if geometric_model=='affine':
            self.gridGen = AffineGridGen(out_h, out_w, use_cuda=use_cuda)
    ...
    def __call__(self, image_batch, theta_batch=None, padding_factor=0.0, crop_factor=1.0):
        ...
        # Ensure theta_batch is on the correct device
        if self.use_cuda:
            theta_batch = theta_batch.to('cuda')
        else:
            theta_batch = theta_batch.to('cpu')
        
        sampling_grid = self.gridGen(theta_batch)

        # Ensure sampling_grid is on the same device as image_batch
        if self.use_cuda:
            sampling_grid = sampling_grid.to('cuda')
            image_batch = image_batch.to('cuda')
        else:
            sampling_grid = sampling_grid.to('cpu')
